...
//			fout << std::fixed;
			...
			fout << "{\n";
			fout << "  \"vertex_count_per_triangle\": " << s_vertexCountPerTriangle << ",\n";
			fout << "  \"vertex_data_count\": " << i_vertexArray.size() << ",\n";
			fout << "  \"indice_data_count\": " << i_indexArray.size() << ",\n";

			fout << "  \"vertex_data\": [\n";
			for (size_t i = 0; i < i_vertexArray.size(); ++i)
			{
				const auto& vertex = i_vertexArray[i].vertex;
				fout << "    {\n";
				fout << "      \"vertex_position\": ["
